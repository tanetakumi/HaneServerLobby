    ...
    public boolean openConnection(){
        try{
            ...
            synchronized ( this ) {
                ...
                //preparedStatement.addBatch("CREATE TABLE IF NOT EXISTS athletics( athletic varchar(10) primary key, player_name varchar(20) ,time int, num int);");
                ...
                //stmtForDeleteAt = connection.prepareStatement("DELETE FROM athletics WHERE uuid = ?;");

            }
            System.out.println("[HaneServerLobby] MySQLに正常に接続できました。");
            return true;
        } catch(SQLException | ClassNotFoundException e){
            System.out.println("[HaneServerLobby] MySQLに正常に接続できませんでした。");
            e.printStackTrace();
            return false;
    ...
    public void closeDisconnect(){
        try{
            if (connection != null && !connection.isClosed()) {
                ...
                System.out.println("[HaneServerLobby] MySQLの接続が正常に終了できました。");
            } else {
                System.out.println("[HaneServerLobby] すでにMySQLの接続がありませんでした。");
            }
        } catch (SQLException e){
            System.out.println("[HaneServerLobby] MySQLの接続が正常に終了できなかったようです。");
            e.printStackTrace();
        }
    }
